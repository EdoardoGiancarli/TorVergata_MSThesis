...
import PySpectr_v4 as psr                       # signals spectrograms
...
def img_generator(N, clean_imgs_path, noisy_imgs_path):
    ...
    for n in tqdm(range(N)):
        ...
        try:
            ...
            if len(np.where(y_clean == 0)[0])/len(y_clean) < 0.1:
                ...
                # spectrograms
                psr.spectr(y_clean, params['dt'], 1024, title = 'clean_spectr' + str(s),
                           images = True, directory = clean_imgs_path)
                
                psr.spectr(y_noisy, params['dt'], 1024, title = 'noisy_spectr' + str(s),
...
class DnsCNetTools:
    ...
    def test_model(self, test_dataset, model = None):
        ...
        print('The mean loss value for the test dataset is:')
