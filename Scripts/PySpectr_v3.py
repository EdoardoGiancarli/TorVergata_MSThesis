...
def spectr(data, dt, lfft, title = None, images = False, directory = None):
    ...
    # spectrogram
    F, T, spectrogram = signal.spectrogram(ifftshift(data), fs=1/dt, window = 'cosine', nperseg = lfft - 1,
                                           noverlap=lfft//2, nfft=lfft, scaling='density', mode='psd')
    ...
    for j in tqdm(range(n_im)):
        ...
        try:
            ...
            if len(np.where(y == 0)[0])/len(y) < 0.1:
                ...
                spectr(y, params['dt'], lfft, title = 'signal_spectrogramzzz_' + str(s),
