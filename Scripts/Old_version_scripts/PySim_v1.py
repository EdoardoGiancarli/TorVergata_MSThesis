...
class GW_injection:
    ...
    Methods:
        MJD2isoUTC: converts the MJD date in input to its iso-UTC format
    ...
    def __init__(self, parameters, amp): #!!!
        ...
        self.tini_iso_gout = self.MJD2isoUTC(self.t0_gout)                             # starting time of the interferometer data [date iso-UTC]
        self.tend_iso_gout = self.MJD2isoUTC(self.t0_gout + int(self.Deltatime_data))  # ending time of the interferometer data [date iso-UTC]
                
    
    ######## methods
    
    
    def MJD2isoUTC(self, mjd_date):
        """
    ...
    def data_chunk(self):
        ...
        if not (self.t0_signal > self.t0_gout and self.t0_signal < self.t0_gout + self.Deltatime_data):
            raise ValueError(f"""The start time of the simulated long transient signal is not in the 
                             observation time interval: t0signal is {self.t0_signal} (MJD) - {self.t0_iso_signal} (iso-UTC),
                             Dtgout is [{self.t0_gout}, {self.t0_gout + int(self.Deltatime_data)}] (MJD) -
                              [{self.tini_iso_gout}, {self.tend_iso_gout}] (iso-UTC).""")
    ...
    def injection(self, plot = False, values = 'real'):  # si potrebbe fare anche con h0factor, da vedere
        ...
        Par:
            ...
            values (str): it specifics the values of s(t) to plot ('real', 'imag' or 'abs', default = 'real')
        return:
        ...
        print(f"""Check on the reciprocal amplitudes: max_h (abs) is {self.amp*np.max(np.abs(obs_h))} 
              and max_noise (abs) is {np.max(np.abs(y_chunk))}""")
