...
class GW_signal:
    ...
    Methods:
        ...
        EinsteinDelay: compute the Einstein delay and its influence on Phi(t) #[6, 7]# !!!
    ...
    Ref:
        ...
        [7] Skyfield (url: https://rhodesmill.org/skyfield/) #!!!
    """
    ...
    def EinsteinDelay(self): #!!!
        ...
        #                            distance = self.NS_parameters[2], frame='icrs')
        
        # # position of source and observer
        # t = ts.from_astropy(obs_time)
        # obs_position = obs_location.get_itrs(obstime = obs_time)
        # source_position = source_location.transform_to(ITRS(obstime = obs_time, location = obs_location)).cartesian.xyz.to(u.m)
        
        # source_distance = self.NS_parameters[2].to(u.m)  # distance in meters
        
        # # Einstein delay
        # einstein_delay = -(2*(G)*M_sun/(c)**3)*np.log(1 + 7)
        
        
        # # position of Earth and Sun
        # earth = eph['earth'].at(obs_time).position.au
        # sun = eph['sun'].at(obs_time).position.au
        ...
        # dist_sun = pos_earth.separation_3d(pos_sun).to(u.m).value
    ...
    def Phi(self, plot = False):   # !!! inserire Einstein delay (here or in a method inside the class)
        ...
        # Einstein delay (the - sign is in the EinsteinDelay() output)
        if self.Einstein_delay:
            p += 2*np.pi*np.cumsum(self.frequency())*self.EinsteinDelay()  
    ...
    def to_pdDataframe(self, objects = 'data', save_to_csv = False, directory = None):   # !!!
        ...
        if objects == 'data':  # makes the data dataframe
...
class GW_injection:
    ...
    def __init__(self, parameters, amp): #!!!
        ...
        if not 'bsd_gout' in parameters.keys():
            raise ValueError("In order to inject the signal you have to set to 'True' the 'signal_inj' input in the function parameters")
        else:
    ...
    def data_chunk(self):
        ...
        if not (self.t0_signal > self.t0_gout and self.t0_signal < self.t0_gout + self.Deltatime_data):
            raise ValueError(f"""The start time of the simulated long transient signal [{self.t0_signal}] is not in the 
                             observation time interval [{self.t0_gout}, {self.t0_gout + int(self.Deltatime_data)}] (MJD date format).""")
        
        else:
            # index for y_gout
            ind = (self.t0_signal - self.t0_gout)*86400/self.dt
            
            # take the length of the obs h_t
            obs_h = self.frequency_band()[2]
            
            # detector's data chunk in which the signal would be observed
            y_chunk = self.y_gout[ind:len(obs_h)]   
        
        return y_chunk


    def injection(self):  # si potrebbe fare anche con h0factor, da vedere
        """
        ...
        # injection
        y = self.amp*obs_h + y_chunk #!!! amp o h0factor
        ...
        # check amp
        print(f"Check on the reciprocal amplitudes: max_h (real) is {self.amp*np.max(obs_h.real)} and max_noise (real) is {np.max(y_chunk.real)}")
    ...
    def to_pdDataframe(self, objects = 'data', save_to_csv = False, directory = None):   # !!!
        ...
        if save_to_csv:
            if directory is None:
                ...
                df.to_csv(directory + 'LTGW_NoiseParameters_' + self.signal_date + '.csv')
