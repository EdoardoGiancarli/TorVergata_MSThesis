# -*- coding: utf-8 -*-
"""
Created on Sat Jun 17 20:02:45 2023

@author: Edoardo Giancarli
"""

#### ModuleMSThesis - Spectrograms simulation version 4 ###############################################
...
def spectr(data, dt, lfft, title = None, images = False, directory = None):
    ...
    # spectrogram
    _, T, spectrogram = signal.spectrogram(data, fs=1/dt, window = 'cosine', nperseg = lfft - 1,
...
def spectrograms_noise(n_im, path_bsd_gout, lfft, images = False, directory = None,
    ...
    if n_im > (len(data)//timelenght_chunk):
        ...
        for j in tqdm(range(n_im)):
            ...
            data_chunk = data[j*timelenght_chunk : (j + 1)*timelenght_chunk]
...
def spectrograms_injsignal(n_im, lfft, images = False, directory = None, store_results = False, save_to_csv = False):
    ...
    for j in tqdm(range(n_im)):
        ...
        amp = np.random.randint(1, 100)*1e-3
        ...
        try:
            ...
            if len(np.where(y == 0)[0])/len(y) < 0.1:
                ...
                spectr(y, params['dt'], lfft, title = 'signal_spectrogram_' + str(s),
