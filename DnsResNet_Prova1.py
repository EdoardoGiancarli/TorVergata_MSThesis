...
model, train_loss3, valid_loss3 = dnt._quicker_train(3, 1, 5, model, 30, 1e-3, train_dataset, valid_dataset, stage_step=1,
                                                     act_func='ReLU', stoch_depth=False, prob = 0.5)

model, train_loss4, valid_loss4 = dnt._quicker_train(3, 2, 8, model, 50, 1e-4, train_dataset, valid_dataset, stage_step=0,
                                                     act_func='ReLU', stoch_depth=False, prob = 0.5)

model, train_loss5, valid_loss5 = dnt._quicker_train(3, 1, 8, model, 50, 1e-5, train_dataset, valid_dataset, stage_step=0,
                                                     act_func='ReLU', stoch_depth=False, prob = 0.5)
...
# ax.plot(x, valid, c = 'LawnGreen', label='valid. loss')
ax.scatter(x, valid, c = 'LawnGreen', label='valid. loss')
# plt.xlabel(f'stages ({epochs} epochs each)')
# plt.xlabel('stages (variable epochs)')
plt.ylabel('loss')
...
# dnt.show_model(comp_dloss = False)
