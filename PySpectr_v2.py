...
def spectrograms_injsign(n_im, path_bsd_gout, path_bsd_gsinj, amp, lfft,
    ...
    for j in range(n_im):
        ...
        if images:
            ...
            plt.pcolormesh(T/86400, fftshift(F), np.log10(fftshift(spectrogram, axes=0)), cmap='Wistia', shading='gouraud')
            plt.axis('off')
            plt.title('')
            ...
            a = plt.pcolormesh(T/86400, fftshift(F), np.log10(fftshift(spectrogram, axes=0)), cmap='Wistia', shading='gouraud')
            plt.colorbar(a)
            plt.title(im_title)
