...
#### ModuleMSThesis - Signals simulation/injection version 3 ###############################################
...
def parameters(days, dt, fgw0, tcoe = 0, n = 5, k = None, tau = 1, Phi0 = 0, right_asc = '05 34 30.9',
    ...
    if isinstance(right_asc, str) and isinstance(dec, str):
        ...
        raise ValueError("Right ascension and declination must be in [hourangle] and [deg] (str) or [decimal deg]\n",
                         "and [decimal deg] (float)")
    ...
    if signal_inj:
        if bsd_gout is None:
            ...
            bsd_gout = pu.extract_bsdgoutinfo(bsd_gout, key=key, mat_v73=mat_v73)
...
class GW_signal:
    ...
    def h0(self):
        ...
        a = self.h0factor*1.0525e-30*(self.NS_parameters[0]/1e38)*(self.NS_parameters[1]/1e-6)*(1/self.NS_parameters[2])
    ...
    def spectr(self, freq_band, lfft = 1024, image = False, img_name = None, directory = None,
               normalisation = True, norm_value = None, sim_noise = False, amp_noise = 1e-6):
        """
        It generates the spectrogram of the simulated data.
        --------------------------------
        Par:
            freq_band (list, ndarray): frequency band of the spectrogram in [Hz]
            ...
            img_name (str): image name (default = None)
            directory (str): directory where to save the spectrograms image (default = None)
            normalisation (bool): spectrogram normalisation (default = True)
            norm_value (int, float): normalisation through specific value (default = None)
            sim_noise (bool): if True Gaussian + Poisson noise is added to the spectrogram (default = False)
            amp_noise (int, float): amplitude of the noise (default = 1e-6)
        ...
        # check on frequency band and dt
        if (1./(freq_band[1] - freq_band[0])) != self.dt:
            raise ValueError(f"the frequency band must be 1/dt = {1/self.dt} Hz broad.")
        
        # select data chunck in the input frequency band
        try:
            # define frequency and extract the frequency band
            signal_frequency = self.frequency()
            sel_freq = signal_frequency[(signal_frequency > freq_band[0]) & (signal_frequency < freq_band[1])]
            
            # indexes of the frequency interval
            a = np.where(signal_frequency == sel_freq[0])[0][0]
            z = np.where(signal_frequency == sel_freq[-1])[0][0]
            
            # extract h_t
            sel_h = self.h_t()[a:z + 1]
            sel_t = self.t[a]
            
        except ValueError:
            raise ValueError(f"The simulated signal has no frequency values in the band [{freq_band[0]}, {freq_band[1]}][Hz].")
        
        # make spectrogram
        _, t, spectrogram = signal.spectrogram(sel_h, fs=1/self.dt, window = 'cosine', nperseg = lfft - 1,
                                               noverlap=lfft//8, nfft=lfft, return_onesided=False,
                                               scaling='density', mode='psd')
        
        if sim_noise:
            spectrogram += 0.5*amp_noise*np.abs(np.random.normal(size=spectrogram.shape))
            spectrogram += 0.5*amp_noise*np.abs(np.random.poisson(size=spectrogram.shape))
        
        if normalisation:
            if norm_value is None:
                spectrogram /= np.max(spectrogram)
            else:
                spectrogram /= norm_value
        ...
        spectr_trs = np.vstack((spectr_shft[int(lfft/2):, :], spectr_shft[0:int(lfft/2), :]))
        
        # define freq array
        freq_trs = np.arange(0, lfft)/(self.dt*lfft) + freq_band[0]
        ...
        if image:                                        # spectrogram image
            ...
            plt.axis('off')
            plt.title('')
            plt.savefig(directory + img_name + '.png', bbox_inches='tight', pad_inches=0)
            plt.close()
            ...
            col_bar = plt.pcolormesh((t + sel_t)/86400, freq_trs, np.log10(spectr_trs + 1e-15),
                                     cmap='viridis', shading='gouraud')
            plt.colorbar(col_bar)
            plt.title(title)
            plt.ylabel('frequency [Hz]')
            plt.xlabel('time [days]' + ' - start: ' + self.signal_date + 'UTC')
            plt.ylim([freq_band[0], freq_band[1]])
            plt.show()
            
        return freq_trs, (t + sel_t)/86400, spectr_trs
...
class GW_injection:
    ...
    Attributes:
        ...
        gws (class): GW_signal module for long transient simulation
    ...
    def data_chunk(self):
        ...
        if (self.t0_signal > self.t0_gout and self.t0_signal < self.t0_gout + self.Deltatime_data):
    ...
    def injection(self, amp=1, plot = False, values = 'real'):
        """
        Injection of the signal in the interferometer's data.
        --------------------------------
        Par:
            amp (float, int): amplitude of the signal wrt noise (default = 1)
        ...
        obs = self.frequency_band()
        obs_h, obs_t = obs[2], obs[3]
    ...
    def spectr(self, amp = 1, lfft = 1024, plot = False, image = False, directory = None,
               normalisation = False, norm_value = None, _img_gen=False,
               _namefig='spectr_s(t)', _offset=False):
        """
        ...
        x = self.injection(amp)
        
        # only for spectrograms generation
        if _img_gen is False:
            pass
        elif _img_gen and len(np.where(x == 0)[0])/len(x) < 0.1:
            pass
        else:
            return None
        ...
        if image and not plot: # spectrogram image
            ...
            plt.pcolormesh(t, freq, np.log10(spectr + 1e-15), cmap='viridis',
                           vmin=-13, vmax=-2, shading='gouraud')
            plt.axis('off')
            plt.title('')
            plt.savefig(directory + _namefig + '.png', bbox_inches='tight', pad_inches=0)
            plt.close()
        
        if plot and not image:
        
            if amp == 1:
                title = 'Spectrogram of $s(t) = h(t) + n(t)$'
            else:
                title = f'Spectrogram of $s(t) =$ {self.amp} $h(t) + n(t)$'
